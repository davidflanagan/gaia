 ...
 * TODO
 *
 * rename this refactored version back to mediadb.js so I
 *   can convert apps one at a time to use it.
 *   start off reintroducing it privately for gallery?
 * 
 *  document API changes:

    - addEventListener, new event types
    - scanning now happens automatically and is not part of the API
    - now indexes by date automatically
    - new statechange event replaces ready/unavailable and adds closed
    - close() method (needed for testing)
 ...
 * write tests:
     ...
     Then close teh mediadb (may need to add a method for this)
       ...
       see that a scan happens (scanstart scanend) but that
...
var MediaDB = (function() {
  ...
  MediaDB.READY = 'ready';           // MediaDB is usable
  ...
  MediaDB.CLOSED = 'closed';         // MediaDB has been closed
  ...
  function changeState(media, state) {
    if (media.state !== state) {
      media.state = state;
      dispatchEvent(media, 'statechanged', state);
    }
  }
